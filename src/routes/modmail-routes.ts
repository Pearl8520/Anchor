...
import { closeModmailThread, onModmailEnabled } from "../services/modmail/modmail-relay.js";
import { slugify } from "../services/modmail/modmail-config.js";

// Every Modmail dashboard route, pulled out of guild-routes.ts so this whole module — this file plus
// src/services/modmail/*, the modmail-* managers, commands, schema, task, and website component — stays
// a clean, self-contained unit that can be dropped into any Discord bot on its own.
...
export function registerModmailRoutes(router: Router, client: ConnectionClient): void {
    router.get('/guilds/:guildId/modmail/settings', requireGuildAccess(client), async (req, res) => {
        ...
        res.json({
            ...
            reminderThresholdHours: settings.reminderThresholdHours,
    ...
    router.patch('/guilds/:guildId/modmail/settings', requireGuildAccess(client), async (req, res) => {
        ...
        if (parsed.data.enabled !== undefined) {
            await database.modmailSettings.setEnabled(guildId, parsed.data.enabled);
            if (parsed.data.enabled && onModmailEnabled) await onModmailEnabled(guildId);
        }
        ...
        if (parsed.data.reminderEnabled !== undefined) await database.modmailSettings.setReminderEnabled(guildId, parsed.data.reminderEnabled);
        if (parsed.data.reminderThresholdHours !== undefined) await database.modmailSettings.setReminderThresholdHours(guildId, parsed.data.reminderThresholdHours);
        ...
        if (parsed.data.linkedGuildId !== undefined) {
            ...
            if (newLinkedGuildId !== null) {
                ...
                if (!client.guilds.cache.has(newLinkedGuildId)) {
                    res.status(400).json({ error: `${client.user?.username ?? 'This bot'} isn't in a server with that ID — make sure it's been invited there first.` });
                    return;
                }
        ...
        res.json({
            ...
            reminderThresholdHours: updated!.reminderThresholdHours,
    ...
    router.patch('/guilds/:guildId/modmail/categories/:key', requireGuildAccess(client), async (req, res) => {
        ...
        if (parsed.data.attachmentLogChannelId !== undefined) {
            await database.modmailSettings.setCategoryAttachmentLogChannel(guildId, key, parsed.data.attachmentLogChannelId);
        }
    ...
    router.get('/guilds/:guildId/modmail/blocked', requireGuildAccess(client), async (req, res) => {
        ...
        res.json({ blocked: blocks.map(b => ({ userId: b.userId, blockedBy: b.blockedBy, reason: b.reason, createdAt: b.createdAt })) });
    });
